...
export const initCommunication = () => {
  _connection = new HubConnectionBuilder()
    .withUrl(Config.api.baseRemote + "/communicationsHub", {
  ...
  _connection.start().then(() => console.log("Connected"));
};
export const sendMessage = <TData>(
  messageType: CommunicationMessageType,
  message: TData
) => {
  _connection.invoke(
    "SendMessage",
    new CommunicationMessage(messageType, message)
  );
};
export const sendMessageGeneral = <TData>(
  messageType: string,
  message: TData
) => {
  _connection.invoke(
...
export const subscribe = <TData>(
  messageType: CommunicationMessageType,
  callback: (data: TData) => void
) => {
  _connection.on(messageType, callback);
};
export const subscribeGeneral = <TData>(
  messageType: string,
  callback: (data: TData) => void
) => {
  _connection.on(messageType, callback);
